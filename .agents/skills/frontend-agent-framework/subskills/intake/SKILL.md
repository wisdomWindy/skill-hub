...
11. 对直接变更请求，将 `state.json.module_flow` 初始化为 `null`，并在 `request.md` 和 `artifacts/prd-snapshot.md` 中记录为什么该请求不需要 PRD-only requirement-analysis / requirement-splitting。
12. 参考样例 `docs/requests/prd-material-delivery-config-center/` 时，只复用结构和质量基线：
   - 可参考目录结构
