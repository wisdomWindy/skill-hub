...
- 用户最新样式变更：所有页面改为 Tailwind CSS 风格；禁止出现 `<style>` 标签、内联样式与行内 `style` 属性
...
5. `module-05-tailwind-style-refactor`：全站页面与组件样式迁移到 Tailwind CSS utility-first 表达，移除 Vue SFC `<style>` 块与所有行内样式入口
